# terminal code: "cd Remnote2Org && python Remnote2OrgRoam.py"
...
re_newLine = re.compile(
    "(\\n){3,}"
)  # replace more than 2 newlines with only 2: https://regex101.com/r/9VAqaO/1/
re_remID = re.compile(
    r"((\]\])?\s*\[.*?(\]\[))"
)  # reference: https://regex101.com/r/z9B8Pw/2
# ---------------------------------------------------------------
pbr = ""
if previewBlockRef:
    pbr = "!"
...
for x in RemnoteDocs:
    if (
        x.get("n", False) == 1
        and x.get("_id", False) not in ignoreID
        and x["key"] != []
        and x["key"][0] not in ignoreKey
    ):
...
def main():
    ...
    for dict in allParentRem:
        ...
        printProgressBar(
            i, len(allParentRem), prefix="Progress:", suffix="Complete", length=50
        )
    ...
    print(
        str(len(notCreated))
        + " file/s listed below could not be generated\n"
        + "\n".join(notCreated)
    ) if len(notCreated) > 0 else None
...
def getOrgLanguage(lang):
    lang = lang.lower()
    langList = json.load(
        open(langJsonPath, mode="rt", encoding="utf-8", errors="ignore")
    )
    try:
